...
class AdminPointEarningController(
    ...
    fun expireNow(
        ...
        expirationService.expireMemberEarningsNow(memberId).map { toResponse(pointEarning = it, memberId = memberId) }

    @Operation(
        summary = "포인트 적립건 강제 만료",
        description =
            "실제 만료일과 무관하게 특정 적립건 하나를 지금 즉시 만료 처리한다. " +
    ...
    fun forceExpire(
        ...
        toResponse(
            pointEarning = expirationService.forceExpireEarning(memberId = memberId, earningId = earningId),
            memberId = memberId,
        )
}
