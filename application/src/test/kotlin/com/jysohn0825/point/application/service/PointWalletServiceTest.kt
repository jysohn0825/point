...
private class FakeQueryWalletRepository(
...
class PointWalletServiceTest :
    BehaviorSpec({
        Given("회원의 지갑이 존재할 때") {
            val walletRepository: FakeQueryWalletRepository = FakeQueryWalletRepository(pointWallet(balance = Balance(BigDecimal(1_400))))
            val walletQueryService: PointWalletService = PointWalletService(walletRepository)
        ...
        Given("회원의 지갑이 존재하지 않을 때") {
            val walletRepository: FakeQueryWalletRepository = FakeQueryWalletRepository(null)
            val walletQueryService: PointWalletService = PointWalletService(walletRepository)
