...
class PointEarningExpirationServiceTest :
    BehaviorSpec({
        Given("이미 만료일이 지난 ACTIVE 적립건이 있을 때") {
            ...
            val earningRepository: FakePointEarningRepository = FakePointEarningRepository(listOf(expired))
        ...
        Given("이미 사용되어 EXHAUSTED 상태인 적립건만 있을 때") {
            ...
            val walletRepository: FakePointWalletRepository = FakePointWalletRepository(pointWallet(balance = Balance(BigDecimal.ZERO)))
            val earningRepository: FakePointEarningRepository = FakePointEarningRepository(listOf(exhausted))
            val expirationService: PointEarningExpirationService =
                service(walletRepository = walletRepository, earningRepository = earningRepository)
        ...
        Given("만료 대상 적립건이 없을 때") {
            ...
            val walletRepository: FakePointWalletRepository = FakePointWalletRepository(pointWallet(balance = Balance(BigDecimal(1_000))))
            val earningRepository: FakePointEarningRepository = FakePointEarningRepository(listOf(active))
            val expirationService: PointEarningExpirationService =
                service(walletRepository = walletRepository, earningRepository = earningRepository)
        ...
        Given("아직 만료일이 지나지 않은 ACTIVE 적립건이 있을 때") {
            ...
            val walletRepository: FakePointWalletRepository = FakePointWalletRepository(pointWallet(balance = Balance(BigDecimal(1_000))))
            val earningRepository: FakePointEarningRepository = FakePointEarningRepository(listOf(notYetDue))
            val expirationService: PointEarningExpirationService =
                service(walletRepository = walletRepository, earningRepository = earningRepository)

            When("강제 만료를 실행하면") {
                val result: PointEarning = expirationService.forceExpireEarning(memberId = "member-1", earningId = "not-due-1")
