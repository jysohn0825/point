...
value class RemainingAmount(
    ...
    init {
        requireDomain(value >= BigDecimal.ZERO) { "잔여 포인트는 0 이상이어야 합니다: $value" }
    }
    ...
    fun decrease(amount: BigDecimal): RemainingAmount {
        requireDomain(amount in BigDecimal.ONE..value) { "차감액($amount)은 1 이상 잔여 포인트($value) 이하이어야 합니다." }
    ...
    fun increase(
        ...
        requireDomain(amount > BigDecimal.ZERO) { "복원액은 0보다 커야 합니다: $amount" }
        val increased: BigDecimal = value + amount
        requireDomain(increased <= upTo.value) { "복원 후 잔여 포인트($increased)는 최초 적립액(${upTo.value})을 초과할 수 없습니다." }
