...
class EarningUsageTraceTest :
    FunSpec({
        test("차감액이 0보다 크면 생성할 수 있다") {
            ...
            val trace = EarningUsageTrace(number, BigDecimal(1_000))
        ...
        test("차감액이 0이면 예외가 발생한다") {
            shouldThrow<IllegalArgumentException> { EarningUsageTrace(orderNumber(), BigDecimal.ZERO) }
        }

        test("차감액이 음수이면 예외가 발생한다") {
            shouldThrow<IllegalArgumentException> { EarningUsageTrace(orderNumber(), BigDecimal(-1)) }
        }

        test("같은 값을 가진 EarningUsageTrace는 서로 동등하다") {
            val number = orderNumber()

            EarningUsageTrace(number, BigDecimal(1_000)) shouldBe EarningUsageTrace(number, BigDecimal(1_000))
        }

        test("다른 금액을 가진 EarningUsageTrace는 서로 동등하지 않다") {
            val number = orderNumber()

            (EarningUsageTrace(number, BigDecimal(1_000)) == EarningUsageTrace(number, BigDecimal(2_000))) shouldBe false
        }

        test("copy로 금액만 변경한 새 인스턴스를 만들 수 있다") {
            val trace = earningUsageTrace(amount = BigDecimal(1_000))

            val copied = trace.copy(amount = BigDecimal(500))
