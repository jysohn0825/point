...
class RemainingAmountTest :
    FunSpec({
        test("0 이상의 값이면 정상적으로 생성된다") {
            val remaining = RemainingAmount(BigDecimal(500))

            remaining.value shouldBe BigDecimal(500)
        }

        test("0은 허용한다") {
            val remaining = RemainingAmount(BigDecimal.ZERO)
        ...
        context("decrease") {
            test("1 이상 잔여액 이하의 금액을 차감할 수 있다") {
                val remaining = RemainingAmount(BigDecimal(1_000))

                val result = remaining.decrease(BigDecimal(300))
            ...
            test("잔여액 전부를 차감하면 0이 된다") {
                ...
                val result = remaining.decrease(BigDecimal(1_000))
        ...
        context("increase") {
            test("최초 적립액 이하로 복원할 수 있다") {
                val upTo = pointAmount(BigDecimal(1_000))
                val remaining = RemainingAmount(BigDecimal(700))

                val result = remaining.increase(BigDecimal(300), upTo)
            ...
            test("복원액이 0 이하이면 예외가 발생한다") {
                ...
                shouldThrow<IllegalArgumentException> {
                    RemainingAmount(BigDecimal(700)).increase(BigDecimal.ZERO, upTo)
                }
            }
            ...
            test("복원 후 금액이 최초 적립액을 초과하면 예외가 발생한다") {
                ...
                shouldThrow<IllegalArgumentException> {
                    RemainingAmount(BigDecimal(700)).increase(BigDecimal(301), upTo)
                }
            }
        }
    })
