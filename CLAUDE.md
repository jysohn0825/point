...
- **Named argument**: 메서드 호출 시 인자는 이름을 명시해서 전달한다. 단, 넘기는 인자 표현식이 파라미터명과 동일한 이름의 식별자(변수/프로퍼티)인 경우는 이미 자기 설명적이므로 이름 명시를 생략할 수 있다 (예: `canAccept(balance, pointAmount)`, `request.to(memberId, usageId)`). 리터럴 값이나 파라미터명과 다른 이름의 표현식을 넘길 때는 반드시 이름을 명시한다 (예: `test(a = 1000)`).
