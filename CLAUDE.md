...
- `application` — 서비스 계층. `domain`의 포트만 의존.
- `infrastructure` — JPA 영속성 어댑터. `domain`의 repository 인터페이스 구현.
...
`infrastructure` 모듈 테스트 중 컨테이너 기반 테스트(`*ContainerTest`)는 Testcontainers(MySQL)를 사용하므로 Docker가 필요하다.
